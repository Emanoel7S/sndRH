    ...
    <main>
        <div class="container-fluid">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <a href="{{ url('ferias/novo') }}" class="btn btn-primary">Novo Pedido</a>
                    <div class="table-responsive">
                        ...
                        @foreach($t as $r)
                            ...
                            <div class="card border-{{ $r->classcss }} mb-3">
                                <div class="card-header bg-transparent border-{{ $r->classcss }}">Pedido de ferias</div>
                                <div class="card-body text-{{ $r->classcss }}">
                                    ...
                                    <p class="card-text">
                                        ...
                                        <strong> D. Inicio: </strong>{{date('d/m/Y', strtotime($r->data_inicio))}} </p>
                                        <strong> D. Fim: </strong>{{date('d/m/Y', strtotime($r->data_fim))}} </p>

                                    <p><strong> Status: </strong>{{$r->status}}</p>
                                    @if($r->aviso)
                                        <p><strong> Aviso: </strong>{{$r->aviso}}</p>
                                    @endif
                                    @if($r->idstatus=='A')
                                        <a href="#" onclick="modal('{{ url('ferias/cancel/' . $r->id) }}')"
